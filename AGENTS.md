...
2. `.github/workflows/nightly.yml` -> `publish-release` job downloads build artifacts, resolves shared metadata + per-system hashes, installs Nix + Cachix, publishes the Linux tarballs and the Darwin tarball when the experimental Darwin matrix leg produced one, then runs `bash scripts/update-nightly.sh`. If the experimental Darwin leg did not upload an artifact, the job preserves the existing `assets."aarch64-darwin"` URL and hash in the manifest so Linux nightly publishing can continue. `workflow_dispatch` can set `publish_release=false` to validate the build matrix without creating a release or committing a manifest bump.
