...
- `modules/_packages/logseq-cli/` also reads `cliPnpmDepsHash`, `cliVendorHash`, and `cliVersion`. The pnpm hash feeds the offline pnpm store; the vendor hash pins the fixed-output nbb dependency source tree copied into `cli/vendor/src`. The CLI builds from source on `x86_64-linux`, `aarch64-linux`, and `aarch64-darwin`; these hashes are currently shared across systems (the vendor tree is pure ClojureScript source, and the pnpm fetch is expected to be platform-independent). If a non-x86_64 build ever reports a pnpm hash mismatch, split `cliPnpmDepsHash` per system in the manifest, validator, and producer together.
...
The Darwin desktop package does not use the FHS wrapper, Linux launcher, icon derivation, or desktop entry. It installs `Logseq.app`, removes copied extended attributes when `xattr` is available, ad-hoc signs the final store app, and exposes `bin/logseq` as a symlink to `Contents/MacOS/Logseq`. Treat Darwin GUI support as experimental until `validate-aarch64-darwin` has passed against a real Darwin asset and a macOS launch has been confirmed.
...
- `modules/_packages/logseq-cli/**`: run `nix build .#logseq-cli` or `nix build .#checks.x86_64-linux.logseq-cli-help`; the smoke check exercises the vendored nbb runtime path. Darwin CLI changes also need GitHub `validate-aarch64-darwin` because the local host is Linux.
