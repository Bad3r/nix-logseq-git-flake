...
- `scripts/install-cachix.sh` is the `installCommand` of every `cachix-action` step. It installs `cachix` from the flake-pinned nixpkgs only when that binary is in `cache.nixos.org`, and otherwise from a fixed nixpkgs rev: the input follows `nixos-unstable`, whose locked rev can lack the `aarch64-darwin` binary, and Nix then compiles cachix from source until the job times out.
...
1. `.github/workflows/nightly.yml` delegates to the reusable `.github/workflows/build-desktop.yml`, which is parameterized by `logseq_repo`/`logseq_ref`, per-arch boolean toggles (`build_x86_64_linux`, `build_aarch64_linux`, `build_aarch64_darwin`), and `apply_patches` (default true; gates the strict patch step). The `build` matrix runs over Linux x64 (`ubuntu-24.04`), Linux arm64 (`ubuntu-24.04-arm`), and Darwin arm64 (`macos-26`); the upstream build runs natively rather than as a Nix derivation, with Nix installed only to supply opam. Each leg clones the upstream repo at the resolved revision, optionally strict-applies `patches/logseq-*.patch`, installs upstream pnpm dependencies, provisions the OCaml/Melange toolchain (opam from the flake-pinned nixpkgs via `nix profile install --inputs-from`, an opam switch on `ocaml-base-compiler.5.1.1`, `opam update`, then `opam install . --deps-only` for the `cli/logseq-cli.opam` closure, plus the `cli/` pnpm workspace), runs `pnpm gulp:build && pnpm cljs:release-electron && pnpm db-worker-node:bundle && opam exec -- pnpm cli:release && pnpm webpack-app-build && pnpm desktop:prepare-runtime-js` to populate `static/` and stage runtime JS under `static/js/`. `opam exec -- pnpm cli:release` runs `dune build @bundle` to produce the OCaml/Melange `logseq-cli.js` that `desktop:prepare-runtime-js` then stages under `static/js/`. The compiler is upstream's `OCAML_VERSION`, built inside the opam switch rather than taken from nixpkgs: the nixpkgs default `ocaml` moves with `flake.lock`, and upstream's `cli/lib/cli.ml` fails on OCaml 5.5 with warning 8 (partial-match) errors on its GADT state machine. Keep it equal to `ocaml-base-compiler` in `opam-deps.nix`. `opam update` runs on every leg because `restore-keys` otherwise carries the first cache's opam-repository snapshot forward indefinitely, leaving the solver unable to see newer releases. Linux runs `pnpm electron:make` and packages `dist/linux*-unpacked` as `logseq-linux-<arch>-<version>.tar.gz`. Darwin verifies `rebuild:all` and `electron:make-macos-arm64`, runs `pnpm rebuild:all && pnpm electron:make-macos-arm64`, copies the single `dist/mac-arm64/*.app` into a clean top-level `Logseq.app` payload, ad-hoc signs it, then packages `logseq-darwin-arm64-<version>.tar.gz`. Each leg writes an SRI hash file and its tarball as artifacts. A separate `metadata` job (parallel to the build matrix) clones the upstream repo, computes version/revision/datestring, renders release notes, and uploads a `build-metadata` artifact containing `meta.txt` and `release-notes.md`; this decouples metadata from any single arch leg so deselecting any leg (including x64) cannot drop shared metadata (matrix job `outputs` are unreliable).
...
- `modules/_packages/logseq-cli/` also reads `cliPnpmDepsHash`, `cliBundlePnpmDepsHash`, `cliCljDepsHash`, and `cliVersion`. The root pnpm hash feeds the offline pnpm store (db-worker + runtime closure: keytar, better-sqlite3, ws, @zvec, ...); `cliBundlePnpmDepsHash` feeds the separate `cli/` pnpm workspace (Vite + transit-js) used by `dune build @bundle`; the Clojure-deps hash pins a fixed-output derivation that resolves the `:cljs` alias classpath (Maven jars under `m2/` plus tools.gitlibs git checkouts under `gitlibs/`) so the db-worker shadow-cljs release compiles offline. The OCaml/Melange closure is resolved by opam-nix from the pinned `opam-repository` flake input plus the `cli/logseq-cli.opam` git pins (`humanize`, `rrbvec`, ...), so it carries no manifest hash. `opam-deps.nix` derives `ocamlBuildInputs` from the resolved root package's `buildInputs` rather than naming each library, so a dependency upstream adds to `depends:` reaches `dune build @bundle` without an edit here; it also checks that `ocaml`, `dune`, and `melange` remain present as structural inputs. Because opam-nix resolves this closure through import-from-derivation (IFD), evaluating `logseq-cli` realizes intermediate opam derivations during eval, so `nix flake check --no-build`/`--offline` over the CLI is not pure eval and needs that opam closure already built or substitutable. The CLI builds from source on `x86_64-linux`, `aarch64-linux`, and `aarch64-darwin`; these hashes are currently shared across systems (the Clojure deps are pure JVM/ClojureScript artifacts, and the pnpm fetches are expected to be platform-independent). `clj-deps.nix` keeps `cliCljDepsHash` shared by stripping the `ignorecase` and `precomposeunicode` keys that git writes into each gitlibs clone's config on Darwin's case-insensitive filesystem. If a non-x86_64 build ever reports a hash mismatch, split the affected hash per system in the manifest, validator, and producer together.
...
- Builds run untrusted code. The reusable `build-desktop.yml` job receives no secrets;
  ...
  to source opam (the OCaml compiler is built inside the opam switch), then strips the GitHub token that `install-nix-action`
...
- PRs touching `.github/workflows/*`, `.github/actions/*`, or `flake.lock` trigger the `validate.yml` `sensitive-change-policy` check, which requires the `status(security-review-approved)` label. The `test-build.yml` `test-build-approval` gate (feeding the required `test-build-required` check) requires the same label for any build-relevant change. Both gates bypass the label when the PR author's `author_association` is `OWNER`, `MEMBER`, or `COLLABORATOR` (owners, org members, and collaborators are trusted; the value is GitHub-computed and cannot be set by the author). The sensitive-path list is defined once in the `.github/actions/detect-sensitive-changes` composite, consumed by `sensitive-change-policy` and by `sensitive-change-review.yml`. `sensitive-change-review.yml` runs on `pull_request_target` and posts, refreshes, or removes an advisory comment on PRs that still need the label (fork PRs cannot be commented on from a `pull_request` run because their token is read-only); it never fails the PR.
